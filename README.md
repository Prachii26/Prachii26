...
### Backend & Systems
`REST APIs` `Microservices` `FastAPI` `Gin`  
`Auth0` `JWT` `RBAC` `API Security`

### AI / ML
`RAG Pipelines` `FAISS` `LLMs` `NLP (spaCy)`  
`PyTorch` `scikit-learn` `Vector Search` `Recommender Systems`
...
- Designed microservice architecture with **RBAC and atomic transactions**
- Deployed on **AWS (EC2, S3, CloudFront)** with Jenkins-based CI/CD
...
- Built **semantic + hybrid search** with spaCy-based NER
- Implemented **Agentic RAG pipelines** using Google Gemini for PDF summarization
